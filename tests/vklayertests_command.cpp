...
TEST_F(VkLayerTest, BadInheritanceInfo) {
    // Ensure that setting VkCommandBufferBeginInfo::pInheritanceInfo to non-null garbage data when calling vkBeginCommandBuffer
    // with a primary command buffer. From
    // https://www.khronos.org/registry/vulkan/specs/1.2-extensions/man/html/VkCommandBufferBeginInfo.html:
    //
    //   pInheritanceInfo is a pointer to a VkCommandBufferInheritanceInfo structure, used if commandBuffer is a secondary command
    //   buffer. If this is a primary command buffer, then this value is ignored.
    //
    // See also https://github.com/KhronosGroup/Vulkan-ValidationLayers/issues/2195.
    // m_errorMonitor->SetDesiredFailureMsg(kErrorBit, "VkCommandBufferBeginInfo::pInheritanceInfo set to non-null garbage should
    // not crash with primary command buffer");

    ASSERT_NO_FATAL_FAILURE(Init());

    // Calls AllocateCommandBuffers for a primary command buffer
    VkCommandBufferObj commandBuffer(m_device, m_commandPool);
    ...
    cmd_buf_info.pNext = NULL;
    cmd_buf_info.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    // Garbage data that should _not_ cause a crash since commandBuffer is primary
    cmd_buf_info.pInheritanceInfo = reinterpret_cast<VkCommandBufferInheritanceInfo *>(0xbad);

    // TODO This is still crashing in safe_VkCommandBufferInheritanceInfo::safe_VkCommandBufferInheritanceInfo as this will get
    // called with
    //      the garbage data in the dispatch call. Looks like this code is generated by scripts/helper_file_generator.py.
    // Begin CB to transition to recording state
    vk::BeginCommandBuffer(commandBuffer.handle(), &cmd_buf_info);
}
